...
- Default DB path (can be overridden via env):
  - `data/premier_league.sqlite` (override via `PL_DB_PATH` or `DB_PATH`)
- DB is initialized by:
  - `scripts/db_init.py`

## Database module
- `src/premier_league/database/`
  - `database/settings.py`: DB path/loading from environment
...
- Formation analytics datasets are derived from:
  ...
  - `teams` (team names)
- Current artifacts are built by `scripts/export_formation_read_models.py` and saved to:
