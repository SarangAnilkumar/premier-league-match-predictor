...
- The log level is controlled by `LOG_LEVEL` loaded from the environment (see `src/premier_league/config.py`).
- The API client (`src/premier_league/clients/api_football.py`) logs:
  - an `info` message when fetching fixtures
- The fixtures ingestor (`src/premier_league/ingestion/fixtures.py`) logs:
  - a full stack trace (`logger.exception(...)`) if fetching fails, then re-raises
- JSON saving is logged (`src/premier_league/io/artifacts.py`) with the output file path.
...
- The schema now includes `ingestion_runs` table (`src/premier_league/database/models.py`).
...
- A read-only audit script (`scripts/db_audit.py`) prints a deterministic health summary from SQLite before/while you ingest larger fixture/lineup batches.
