...
- `scripts/db_audit.py` provides a non-invasive SQLite health report (counts + coverage + completeness metrics) to validate schema/cache/loader behavior as you ingest more data.
...
- Transfers ingestion into SQLite is implemented via `scripts/ingest_transfers.py` (`src/premier_league/ingestion/transfers.py` + `src/premier_league/database/loaders/transfers.py`).
